  ...
  return (
    <div className="lg:sticky lg:top-24 h-fit">
      <Card className="p-6 bg-accent text-accent-foreground">
        <div className="mb-6">
          <h3 className="font-bold text-lg mb-2">ALL INKLUSIVE 🤍 {packageNameMap[packageName]}</h3>
          {guestCount && (
            <p className="text-sm">
          ...
          {guestCount && (
            <p className="text-2xl font-bold mt-2">{(guestCount * basePrice).toFixed(2).replace(".", ",")}€</p>
          )}
        </div>
        ...
        {serviceFee > 0 && (
          <div className="mb-6 pb-6 border-b border-accent-foreground/20">
            ...
            <div className="text-lg font-bold">{serviceFee.toFixed(2).replace(".", ",")}€</div>
          </div>
        )}
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="border-t border-accent-foreground/20 pt-3 mt-3">
            ...
            <div className="text-xs opacity-75 mt-1">inkl. {vat.toFixed(2).replace(".", ",")}€ MwSt.</div>
          </div>
        </div>
      </Card>
    </div>
